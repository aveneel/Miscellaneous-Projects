...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    def comment(score0, score1):
        if who==0:
            new_previous_score=score0-previous_score
            new_previous_high=max(new_previous_score, previous_high)

            if (new_previous_score>previous_high):
                if new_previous_score==1:
                    print(str(new_previous_score) + " point! That's the biggest gain yet for Player 0")
                else:
                    print(str(new_previous_score) + " points! That's the biggest gain yet for Player 0")
            return announce_highest(who, new_previous_high, score0)

        else:

            new_previous_score=score1-previous_score
            new_previous_high=max(new_previous_score, previous_high)

            if (new_previous_score>previous_high):
                if new_previous_score == 1:
                    print(str(new_previous_score) + " point! That's the biggest gain yet for Player 1")
                else:
                    print(str(new_previous_score) + " points! That's the biggest gain yet for Player 1")
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    n=1
    max=1
    highestaverage=make_averaged(roll_dice,100)(1,dice)

    while n<11:
        f=make_averaged(roll_dice,100)(n,dice)
        if f>highestaverage:
            highestaverage=f
            max=n
        n+=1
