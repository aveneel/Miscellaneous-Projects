...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return eval_operator.eval_call(rest, env)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        list_a = operands.map (lambda o: scheme_eval(o, env))

        return scheme_apply(self, list_a, env)
...
def make_let_frame(bindings, env):
    ...
    sympair, valpair = nil, nil

    while bindings is not nil:
        check_form(bindings.first, 2, 2)
        sympair = Pair(bindings.first.first, sympair)
        valpair = Pair(scheme_eval(bindings.first.second.first, env), valpair)
        bindings = bindings.second

    check_formals(sympair)

    return env.make_child_frame(sympair, valpair)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        macro_function = MacroProcedure(target.second, expressions.second, env)
        env.define(target.first, macro_function)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
