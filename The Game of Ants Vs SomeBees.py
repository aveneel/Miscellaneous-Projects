...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while location.entrance != None or location != hive:
            if len(location.bees)> 0 and i >= self.min_range and i <= self.max_range:
...
def apply_effect(effect, bee, duration):
    ...
    def effectedaction(colony):
        nonlocal duration

        if duration:
            duration -= 1
            return effect(originalaction)(colony)
        else:
            return originalaction(colony)

    bee.action = effectedaction
